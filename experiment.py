...
FOOTPRINT_KEYS = {'DATASET_ID', 'TEST_SIZE', 'RANDOM_STATE', 'K_FOLDS', 'SCORING', 'PICTURE_SIZE'}
EXPERIMENT_FOOTPRINT = {k: v for k, v in locals().items() if k in FOOTPRINT_KEYS}
EXPERIMENT_ID = sha512(json.dumps(EXPERIMENT_FOOTPRINT, sort_keys=True).encode()).hexdigest()
...
# by looking for the existence of a .json file named after the EXPERIMENT_ID
...
# Save experiment footprint into OUTPUT_DIR/EXPERIMENT_ID.json
with open(OUTPUT_DIR / f'{EXPERIMENT_ID}.json', 'w') as file:
    json.dump(EXPERIMENT_FOOTPRINT, file, indent=4)
